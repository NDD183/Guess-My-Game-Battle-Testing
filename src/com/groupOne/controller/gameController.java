...
import com.groupOne.model.Case;
...
public class gameController implements Initializable {
    ...
    private  List<Integer> secretGuessList = new ArrayList<>();
    ...
    private Boolean isFirstSecret = true;
    private List<Integer>  testGuess = null;
    private List<String> secretList = new ArrayList<>();
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        // Set place holder
        gnumberField.setPromptText("Enter 4 digits number");
...
//        snumberField.setText(snumber);
        if(!gnumberField.getText().equals("")) {
            if (isFirstSecret) {
                List<String> holder = generateHolder();
                for (int i = 0; i < holder.size(); i++) {
                    List<Integer> result = getResult(stringTransform(holder.get(i)), stringTransform(gnumberField.getText()));
                    if (result.get(0) == 0 && result.get(1) == 1) {
                        secretList.add(holder.get(i));
                    }
                }
                isFirstSecret = false;
            } else {
               secretList =  cleanHolder(stringTransform(gnumberField.getText()));
            }
            int index = ThreadLocalRandom.current().nextInt(0, (secretList.size()));
            snumberField.setText(secretList.get(index));
            System.out.println(snumberField.getText());
        } else {
            System.out.println("Error empty guess");
        }
    }
    /**Name: resultClicked
    ...
    public void resultClicked(ActionEvent actionEvent) {
        ...
        if(isValidNumber(gnumber)) {
            ...
            secretGuessList = stringTransform(snumber);
            // Get result from user input
            infoList = getResult(secretGuessList, rivalGuessList);
    ...
    public void generatengClicked(ActionEvent actionEvent) {
        ...
        int missValue = 4 - strikeValue - hitValue;
...
// -------------------------------------All LOGIC GAME FUNCTIONS START FROM HERE--------------------------------------//
    ...
    public String guessNumber(String currentGuess, List<Integer> result) {
        ...
        // Check if this is the run for the first guess
        // IF YES
        if(isFirst) {
    ...
    public List<Case> generateCase() {
        List<Case> tempCases = new ArrayList<>();
        int i, x;
        for(i = 0; i < 5; i ++) {
            if (i == 0) {
                for(x = 0; x < 5; x ++) {
                    tempCases.add(new Case(x, i, x));
                }
            }
            if(i == 1) {
                for(x = 0; x < 4; x ++) {
                    tempCases.add(new Case(x + 5, i, x));
                }
            }
            if(i == 2) {
                for(x = 0; x < 3; x ++) {
                    tempCases.add(new Case(x + 9, i, x));
                }
            }
            if(i == 3) {
                for(x = 0; x < 2; x ++) {
                    tempCases.add(new Case(x + 12, i, x));
                }
            }
            if(i == 4) {
                tempCases.add(new Case(14, i, 0));
            }
        }
        return tempCases;
    }
    public List<String> cleanHolder(List<Integer> guessNumber) {
        List<Integer> result;
        ...
        for (Case c: caseList) {
            ...
            for(i = 0; i < secretList.size(); i++) {
                result = getResult(stringTransform(secretList.get(i)), guessNumber);
                if (result.get(0) != c.getStrikeValue() || result.get(1) != c.getHitValue()) {
                    count++;
                    //System.out.println(c.getId());
                }
            }
            intArr[c.getId()] = count;
            //tempResult.set(c.getId(), count);
        }
      //  System.out.println(intArr.length);
        for (i = 0; i < intArr.length; i++) {
            if(intArr[i] < minCount) {
                minCount = intArr[i];
                minIndex = i;
            }
           System.out.println(i +": " + intArr[i]);
        }
        System.out.println(minIndex + ": "+ minCount);
        System.out.println(caseList.get(minIndex).getStrikeValue()+ "-" +  caseList.get(minIndex).getHitValue());
        for(i = 0; i < secretList.size(); i++) {
            result = getResult(stringTransform(secretList.get(i)), guessNumber);
            if (result.get(0).equals(caseList.get(minIndex).getStrikeValue())
                    && result.get(1).equals(caseList.get(minIndex).getHitValue())) {
                tempResult.add(secretList.get(i));
            }
        }
        System.out.println("Size: " + secretList.size());
//        secretList.clear();
//        // Loop through the tempHolder
//        for(i = 0; i < tempResult.size(); i++) {
//            // Add each element from tempHolder to holder list
//            secretList.add(tempResult.get(i));
//        }
//        // Clear the tempHolder list
//        tempResult.clear();
        return tempResult;
